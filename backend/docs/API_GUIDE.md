...
2. **単一拠点のみ:** 現在は1拠点のみサポート（複数拠点対応はStory 005で実装予定）
   - VRPソルバーは全車両が `depots[0]` から出発・帰還する前提で動作
   - 複数拠点データを投入しても最初の拠点のみ使用される
3. **重量容量制約のみ:** 車両体積制約は未実装
   - OR-Tools の AddDimensionWithVehicleCapacity で重量のみ制約
   - Demo データは体積利用率27%で設計されており、実用上問題なし
   - 本番環境では体積制約の追加実装が必要
4. **基線統計の簡略化:** 未使用車両を0%積載率として平均計算
   - 改善指標（utilization_improvement_percent）に影響
   - Demo では最適化効果がやや過大評価される可能性あり
5. **同期API:** 長時間計算もブロッキング（30秒上限）
