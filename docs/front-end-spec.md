...
2. **最適化実行中（Optimization in Progress）**
   - 目的：計算プロセスの透明性提供
   - 内容：プログレスバー + ステータステキスト（「30件の配送先を分析中...」）
   - 所要時間：10-60秒（OR-Tools計算時間、平均16秒）
...
graph TD
    ...
    Check -->|No| Load[デモデータ自動ロード<br/>30件配送先+5台車両]
    Check -->|Yes| Ready
...
│              AI自動配車システムデモ                           │
│                                                              │
│       30件の配送先を5台の車両に最適割り当て                  │
│                                                              │
│  ┌────────────────────────────────────────────────────┐   │
│  │                                                    │   │
│  │  ✓ 2拠点から順次積込                               │   │
│  │  ✓ 時間窓制約対応（午前20%、午後30%、指定なし50%）  │   │
│  │  ✓ 容量制約（重量・体積）                          │   │
│  │  ✓ コスト最適化（距離・時間）                      │   │
│  │                                                    │   │
│  └────────────────────────────────────────────────────┘   │
...
2. **单击演示流程**
   - 主页 → 点击"最適化開始" → 计算中 → 结果展示
   - 预加载演示数据（2拠点+5台車両+30配送点）
   - 计算时间目标：10-60秒以内（平均16秒、Epic 005優化）
