...
8. **Multi-Depot VRP統合準備**
   - 生成された4拠点が VRP エンジンの Multi-Depot 入力として使用可能
   - 各拠点に最低1台の車両が配分されることを確認（Story 5.2で実装）
   - 配送先の `depot_id` 設定が正確（最寄り拠点に紐付け）
   - **テストで各拠点から最低1ルートが生成されることを検証**（Story 5.2完了後）
