...
> **🚧 プロジェクト状態：** Epic 005 完了（Multi-Depot VRP対応実装完了）
> **📅 最終更新：** 2025-11-04
> **✅ 現在の実装：** Epic 005 完了（2拠点・30配送先・5台車両 Multi-Depot VRP）
> **🔜 開発予定：** E2Eテスト、パフォーマンス最適化

---

## 📋 プロジェクト概要

日本の物流業界向けのAI自動配車システムのデモンストレーション用プロトタイプ。**2つの集荷拠点、30件の配送先、5台の車両**を使用した最適化配車のデモンストレーション（Epic 005対応）。
...
**Epic 005対応版（2拠点、5台車両、30配送先）：**

> **注記：** Epic 005（Story 5.1-5.3）では2拠点・30配送先・5台車両のMulti-Depot VRP実装完了。実測値は統合テスト後に更新予定。
...
### Story 005: Epic 005 - Demoデータ拡張と最適化 ✅ 完了（2025-11-04）

#### Story 5.1: 多拠点・中規模配送先データ生成 ✅ 完了
- ✅ 2拠点の定義と生成（東京・さいたま市）
- ✅ 30件の配送先生成（固定配置法による実在地点使用）
- ✅ 伝票枚数分布対応（1枚50%・2枚35%・3枚15%）
- ✅ 時間窓分布対応（午前20%・午後30%・指定なし50%）

#### Story 5.1.1: データ生成最適化と拠点制約実装 ✅ 完了
- ✅ 海上配置問題の完全解消（0件）
- ✅ 配送点数最適化（40件→30件）
- ✅ VRP計算時間大幅短縮（300秒→60秒タイムアウト、平均16秒）

#### Story 5.2: Multi-Depot VRP実装 ✅ 完了
- ✅ 双重容量制約実装（重量・容積）
- ✅ Multi-Depot拠点制約（SetAllowedVehiclesForIndex）
- ✅ 時間窓制約の最適化
- ✅ 平均計算時間16秒以内を達成

#### Story 5.3: UI/UX調整とパフォーマンス検証 ✅ 完了
- ✅ VRP計算進捗表示の実装
...
- **拠点:** 2箇所（東京・さいたま市、約20km離れた2拠点）
- **車両:** 5台（2t車×4、4t車×1）
- **配送先:** 30件（東京20件・さいたま市10件、各拠点から50km範囲内）
- **時間窓:** 午前20%（6件）・午後30%（9件）・指定なし50%（15件）/ Epic 005最適化
